...
setup-comparative:
	$(UV) pip install --python $(PYTHON) -r requirements-comparative.txt
...
run-reference-ablation:
	$(PYTHON) -m src.main run-reference-ablation --config configs/project_config.yml --top-k-values 25,50,100,250
...
build-tcgabiolinks-comparator:
	docker build -t canceromicslake-tcgabiolinks:bioc-3.21 comparative/tcgabiolinks

run-tcgabiolinks-comparison:
	$(PYTHON) -m src.main run-comparative-evaluation --collect-tcgabiolinks

run-comparative-evaluation:
	$(PYTHON) -m src.main run-comparative-evaluation --collect-local --collect-cbioportal --collect-xena

run-comparative-evaluation-containerized:
	docker compose build dbt
	docker compose run --rm -e CANCEROMICSLAKE_GIT_COMMIT=$(SOURCE_COMMIT) dbt python -m src.main run-comparative-evaluation --collect-local

run-comparative-evaluation-strict:
	$(PYTHON) -m src.main run-comparative-evaluation --strict

run-submission-readiness:
	$(PYTHON) -m src.main run-submission-readiness

run-submission-readiness-strict:
	$(PYTHON) -m src.main run-submission-readiness --strict
