...
def test_research_bootstrap_stability_endpoint() -> None:
    response = client.get("/research/bootstrap-stability", params={"limit": 5})
    assert response.status_code == 200
    payload = response.json()
    assert "rows" in payload
    assert payload["filters"]["limit"] == 5
...
def test_research_consensus_candidates_endpoint() -> None:
    response = client.get("/research/consensus-candidates", params={"limit": 5})
    assert response.status_code == 200
    payload = response.json()
    assert "rows" in payload
    assert payload["filters"]["limit"] == 5
...
def test_research_paired_expression_support_endpoint() -> None:
    response = client.get("/research/paired-expression-support", params={"limit": 5})
    assert response.status_code == 200
    payload = response.json()
    assert "rows" in payload
    assert payload["filters"]["limit"] == 5
